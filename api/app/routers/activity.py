...
def create_activity(
    ...
    countries = db.query(Country).filter(Country.id.in_(body.countryIds)).all()
    if len(countries) != len(body.countryIds):
        existing = {c.id for c in countries}
        missing = [cid for cid in body.countryIds if cid not in existing]
        raise HTTPException(
...
def update_activity(
    ...
    country_ids = update_data.pop("countryIds", None)
